...
在“买家发现”页填写主营产品、必中规格/品类、目标市场、目标买方类型、最低等级与目标数量，引擎会自动生成多渠道检索方案 → 发现并筛选买家 → 按“匹配度+实力”双维度分级（S/A/B/C）→ 自动补齐缺口迭代，完成后可一键导入客户库（保留评分与等级）。也可用命令行 `python scripts/run_acquisition.py --conditions 条件文件.json [--seed 已收集线索.json]` 输出策略文档与目标清单。
